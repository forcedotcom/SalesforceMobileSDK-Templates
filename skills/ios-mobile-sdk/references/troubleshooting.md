...
Symptom-first reference for iOS Mobile SDK integration failures. Each row pairs a verifiable symptom with a unique cause and the corrective change.
...
| `xcodebuild` fails with `Found no destinations for the scheme` | Either `supportedDestinations` is missing on the target (Xcode 16+ excludes the simulator unless declared) or the scheme is not shared at `<AppName>.xcodeproj/xcshareddata/xcschemes/<AppName>.xcscheme`. | For xcodegen-managed projects (see [`create-new-app.md`](create-new-app.md)): set `supportedDestinations: [iOS, iPadOS, iOSSimulator]` on the target and add a top-level `schemes:` block in `project.yml`, then `xcodegen generate`. For hand-maintained `.xcodeproj` (the pattern used by `iOSNativeSwiftTemplate`): confirm a shared scheme exists, and pass `-destination 'platform=iOS Simulator,name=iPhone 17 Pro'` (or `'generic/platform=iOS Simulator'`) to `xcodebuild`. |
| New file added on disk does not appear in the build | The file was created on disk but not added to the Xcode target. | For xcodegen projects: re-run `xcodegen generate` (then `pod install` if using CocoaPods, since `xcodegen generate` rewrites `.xcodeproj`). For hand-maintained `.xcodeproj`: drag the file into the project navigator and tick the app target's Target Membership. For resources (e.g. `userstore.json`, `usersyncs.json`, `bootconfig.plist`), confirm the file is in Build Phases → Copy Bundle Resources. |
...
| Symptom | Cause | Fix |
|---|---|---|
| Biometric prompt does not appear | `NSFaceIDUsageDescription` missing in `Info.plist`, or the Salesforce Connected App backing the consumer key has no Mobile App Policy enabling biometric auth — so `SalesforceManager.shared.biometricAuthenticationManager().enabled` is `false`. | Add `NSFaceIDUsageDescription` to `Info.plist`. Configure a Mobile App Policy on the Connected App with biometric auth enabled and a non-zero session timeout. There is no app-side switch that turns the feature on. |
| Opt-in dialog appears every launch even after the user opted in | The opt-in is being presented while the SDK has the session locked (post-relaunch, pre-unlock), or while the user has already opted in. | Guard with both `enabled`, `!locked`, and `!hasBiometricOptedIn()` before calling `presentOptInDialog(viewController:)`. Or set `automaticPresentation = true` (the default) and let the SDK manage opt-in itself. |
| Compile error: `BiometricAuthenticationManager` has no member `shared` (or no member `biometricAuthenticationEnabled` / `biometricAuthenticationPolicy` / `showBiometricEnrollment`) | `BiometricAuthenticationManager` is a Swift `protocol` (annotated `@objc(SFBiometricAuthenticationManager)`) — there is no static `.shared` and none of those property names exist on the protocol. | Access the singleton via `SalesforceManager.shared.biometricAuthenticationManager()` (Obj-C method, parenthesized in Swift). The full public surface is in [`add-biometric-auth.md`](add-biometric-auth.md). |

## Code Signing

| Symptom | Cause | Fix |
|---|---|---|
| Login completes in UI but tokens vanish on relaunch (or never set) | `CODE_SIGNING_ALLOWED=NO` was passed to `xcodebuild`, stripping entitlements. | Drop the flag. Use ad-hoc signing — the template's `CODE_SIGN_IDENTITY=-` is sufficient for simulator builds. |
