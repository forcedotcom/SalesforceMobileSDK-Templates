...
| `SyncState` | `com.salesforce.androidsdk.mobilesync.util` | Sync result/state object |
...
| `PushService` | `com.salesforce.androidsdk.push` | Push registration entry point — subclass `PushService` to handle Salesforce push registration and trigger registration via the `PushMessaging` Kotlin object. The `AndroidNativeKotlinTemplate` ships a `PushNotificationsAdapter` example that implements `PushNotificationInterface`. |
...
- Working examples: <https://github.com/forcedotcom/SalesforceMobileSDK-Templates/tree/dev/AndroidNativeKotlinTemplate>
