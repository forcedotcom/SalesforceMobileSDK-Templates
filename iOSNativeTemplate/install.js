...
for (var sdkDependency in packageJson.sdkDependencies) {
    ...
    if (fs.existsSync(targetDir))
        ...
        execSync('git clone --branch ' + branch + ' --single-branch --depth 1 --recurse-submodules=external/fmdb --shallow-submodules ' + repoUrl + ' ' + targetDir, {stdio:[0,1,2]}); // we should move the submodule pathspec to package.json
}
